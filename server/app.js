...
app.put("/api/chat/messages/:messageId/read", verifyToken, (req, res) => {
  try {
    const db = readDb();
...
app.get("/api/assignments", verifyToken, (req, res) => {
  try {
    ...
    const assignments = db.assignments.map(assignment => {
      ...
      return {
        ...assignment,
        teacher: teacher ? { id: teacher.id, name: teacher.name, subjects: teacher.subjects, standards: teacher.standards } : null,
        student: student ? { id: student.id, name: student.name, standard: student.standard, medium: student.medium } : null
      };
    });
    ...
    if (req.user.role === "teacher") {
      filteredAssignments = assignments.filter(a => a.teacherId === req.user.id);
    } else if (req.user.role === "student") {
      filteredAssignments = assignments.filter(a => a.studentId === req.user.id);
    }
    
    res.json({ data: filteredAssignments });
  } catch (error) {
    res.status(500).json({ message: "Failed to fetch assignments" });
  }
});

app.post("/api/assignments", verifyToken, allowRoles(["admin"]), (req, res) => {
  try {
    const { teacherId, studentId, subject } = req.body;
    
    if (!teacherId || !studentId || !subject) {
      return res.status(400).json({ message: "Teacher ID, Student ID, and Subject are required" });
    }
    ...
    const existingAssignment = db.assignments.find(a => 
      a.teacherId === teacherId && a.studentId === studentId && a.subject === subject
    );
    ...
    // Verify teacher teaches this subject and student is in teacher's standard
    if (!teacher.subjects.includes(subject)) {
      return res.status(400).json({ message: "Teacher does not teach this subject" });
    }
    
    if (!teacher.standards.includes(student.standard)) {
      return res.status(400).json({ message: "Teacher does not teach this student's standard" });
    }
...
app.delete("/api/assignments/:assignmentId", verifyToken, allowRoles(["admin"]), (req, res) => {
  try {
...
app.get("/api/teacher/students", verifyToken, allowRoles(["teacher"]), (req, res) => {
  try {
    ...
    const students = db.users
      ...
      .filter(student => 
        // Show students who are in teacher's standards and medium
        teacher.standards.includes(student.standard) && 
        teacher.medium === student.medium
      )
      .map(student => ({
        id: student.id,
        name: student.name,
        ...
        assignments: teacherAssignments
          ...
          .map(a => ({ subject: a.subject, assignmentId: a.id }))
      }));
...
app.get("/api/student/teachers", verifyToken, allowRoles(["student", "admin"]), (req, res) => {
  try {
    const db = readDb();
    ...
    // Get all teachers who teach this student's standard and medium
    const availableTeachers = db.users
      .filter(u => u.role === "teacher" && u.status === "approved")
      .filter(teacher => 
        teacher.standards.includes(student.standard) && 
        teacher.medium === student.medium
      )
      .map(teacher => {
        const assignments = db.assignments.filter(a => 
          a.teacherId === teacher.id && a.studentId === studentId
        );
        return {
          id: teacher.id,
          name: teacher.name,
          ...
          assignments: assignments.map(a => ({ subject: a.subject, assignmentId: a.id }))
        };
      });
    
    res.json({ data: availableTeachers });
  } catch (error) {
